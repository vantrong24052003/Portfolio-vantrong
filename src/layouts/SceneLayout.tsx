...
export const SceneLayout: React.FC<SceneLayoutProps> = ({ children }) => {
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-background selection:bg-primary/30 selection:text-foreground">
      ...
      {/* Content */}
      <main className="relative z-10 w-full min-h-screen animate-fade-in">
        {children}
      </main>
      ...
      <div className="absolute top-20 right-20 w-96 h-96 bg-accent rounded-full mix-blend-overlay filter blur-[120px] opacity-20 pointer-events-none z-0" />
    </div>
  );
};
